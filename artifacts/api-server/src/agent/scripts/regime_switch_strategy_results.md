# Backtest: filtro de regime (SMA100) liga/desliga a estratégia de sinal europeu

Testa se um filtro de tendência simples (Close do Nasdaq vs. sua própria SMA100) detecta automaticamente o mesmo regime que rotulamos na mão em backtest_global_signal_strategy.py -- e se o híbrido resultante (buy&hold passivo em alta, sinal europeu fora de alta) preserva o edge da correção sem perder tanto no rali. Retorno/Sharpe reportados BRUTO (sem custo) e LÍQUIDO (custo igual a backtest.py, cobrado a cada troca de posição, inclusive a troca causada pelo próprio filtro de regime).
