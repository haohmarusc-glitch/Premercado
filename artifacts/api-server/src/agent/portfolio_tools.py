...
def get_positions() -> List[dict]:
    try:
        r = requests.get(
            f"{_api_url()}/api/portfolio/positions",
            headers=_headers(),
            timeout=10,
        )
        r.raise_for_status()
        ...
        for pos in positions:
            ...
            purchase = pos.get("purchasePrice") or pos.get("purchase_price") or 0
...
PORTFOLIO_TOOLS = [
    {
        ...
        "description": "Retorna posições da carteira com preço de compra, preço atual, P&L por posição.",
...
PORTFOLIO_DISPATCH = {
    "get_portfolio_positions": get_positions,
