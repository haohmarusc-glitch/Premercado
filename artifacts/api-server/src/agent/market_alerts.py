...
MACRO_EVENTS: dict[str, list[str]] = {
    "CPI":  ["2026-06-10", "2026-07-15"],
    "FOMC": ["2026-06-17", "2026-07-29"],
}
...
def check_macro_triggers(today: Optional[dt.date] = None) -> list[Alert]:
    ...
    if today.weekday() == 4 and today.day <= 7:
        alerts.append(Alert(
            ...
            title="Dia de Payroll",
            detail="Relatorio de empregos dos EUA hoje. Surpresa positiva -> juros sobem "
                   "-> nomes de multiplo alto (SMCI, etc.) sob maior risco.",
        ))

    for nome, datas in MACRO_EVENTS.items():
        for ds in datas:
            try:
            ...
            if 0 <= delta <= MACRO_WINDOW_DAYS:
                ...
                alerts.append(Alert(
                    ticker="MACRO", category=Category.MACRO, severity=Severity.ATENCAO,
                    title=f"Evento macro: {nome}",
                    detail=f"{nome} {quando} ({ds}). Esperar volatilidade; rebaixar confianca de sinais fortes.",
                ))
    ...
    if df is not None and not df.empty:
        ...
        if y >= YIELD_LEVEL:
            alerts.append(Alert(
                ...
                detail=f"10y em ~{y:.2f}% (limiar {YIELD_LEVEL}%). Pressiona valuation de acoes de crescimento.",
                value=round(y, 2),
            ))
    return alerts
...
def check_analyst_changes(ticker: str, headlines: list[str]) -> list[Alert]:
    alerts: list[Alert] = []
...
def check_sell_the_news(ticker: str, headlines: list[str],
