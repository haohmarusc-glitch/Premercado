"""
probe_gemini.py: medir quais modelos do Gemini sustentam o fluxo, em vez de
...
Rodar: pytest artifacts/api-server/src/__tests__/test_probe_gemini.py -v
"""

from agent import probe_gemini as pg
...
class TestProbe:
    ...
    def test_marca_ausencia_de_preco(self, monkeypatch):
        ...
        _instalar(monkeypatch, [_pediu_ferramenta(), _texto(RESPOSTA_LONGA)])
        assert pg.probe("modelo-inexistente-sem-preco")["tem_preco"] is False

        _instalar(monkeypatch, [_pediu_ferramenta(), _texto(RESPOSTA_LONGA)])
        conhecido = next(iter(pg.MODEL_PRICING))
        assert pg.probe(conhecido)["tem_preco"] is True

    def test_sempre_mede_o_tempo(self, monkeypatch):
        _instalar(monkeypatch, [RuntimeError("boom")])
        assert pg.probe("x")["segundos"] is not None
...
class TestSaida:
    ...
    def _resultado(self, **kw):
        base = {
            "model": "m", "ok": True, "chamou_ferramenta": True,
    ...
    def test_prefere_o_que_nao_vaza_chamada(self, capsys):
        ...
        assert "Sugestão para PROVIDERS['gemini']['models']['full']: limpo" in out
