...
export default function SectorAI() {
  ...
  const runAI = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "⚡ Análise de IA iniciada", description: "NVDA · ARM · GOOGL · META · MSFT · AMD · PLTR · SMCI em análise." });
    },
  });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="border-b border-border pb-4 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold font-mono text-foreground tracking-tight">INTELIGÊNCIA ARTIFICIAL</h1>
          <p className="text-muted-foreground font-mono text-sm mt-1">
            NVDA · ARM · GOOGL · META · MSFT · AMD · PLTR · SMCI
          </p>
          {sectorObs.length > 0 && (
        ...
        <button
          ...
          disabled={isRunning || runAI.isPending}
        ...
        >
          ...
          {isRunning ? "ANALISANDO..." : "ANALISAR IA"}
        </button>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="p-12 text-center border border-dashed border-border rounded-sm text-muted-foreground font-mono text-sm">
          Nenhuma observação para o setor de IA. Clique em <strong>ANALISAR IA</strong> para gerar.
        </div>
      ) : (
